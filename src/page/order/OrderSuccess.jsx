...
const OrderSuccess = () => {
    ...
    if (error) return <p className="text-red-500 text-center mt-20 text-lg">{error}</p>;
    if (!order) return <p className="text-center mt-20 text-gray-500 text-lg">Loading...</p>;
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-6">
            <div className="bg-white p-8 rounded-2xl shadow-xl max-w-3xl w-full animate-fadeIn">
                ...
                <p className="text-gray-900 text-center mb-6">
                    Thank you <span className="font-semibold">{order.shippingAddress?.fullName}</span>, your
                    order has been placed.
                </p>

                {/* Order Details */}
                <section className="mb-6 border-t pt-4">
                    <h2 className="font-semibold text-xl mb-2 border-b pb-1">Order Details</h2>
                    <p>
                        <b>Order ID:</b> {order._id}
                    </p>
                    <p>
                        <b>Status:</b> <span className="text-black">{order.orderStatus}</span>
                    </p>
                    <p>
                    ...
                    <p>
                        <b>Total:</b>{" "}
                        <span className="text-black font-semibold">{formatPrice(order.total)}</span>
                    </p>
                    <p>
                        <b>Delivery To:</b>{" "}
                    ...
                    {order.cartItems.map((item, i) => (
                        <div key={i} className="flex items-center justify-between py-2 border-b">
                            <div className="flex items-center space-x-4">
                                <img
                                    src={item.image?.startsWith("http") ? item.image : IMAGE_URL + item.image}
                                    alt={item.name}
