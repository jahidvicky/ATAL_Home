...
// ---------- NEW: helper to resolve image urls (no removals) ----------
const resolveImg = (src = "") => {
  if (!src) return "/no-image.png"; // fallback placeholder

  // if src already full (absolute URL)
  if (src.startsWith("http://") || src.startsWith("https://")) return src;

  // Ensure IMAGE_URL ends with exactly one slash
  const base = IMAGE_URL.endsWith("/") ? IMAGE_URL : `${IMAGE_URL}/`;

  // remove any leading slashes from src to avoid double slashes
...
function ProductCard({
  ...
  // per-card state now lives inside a component, not in the parent map 
  const [activeVar, setActiveVar] = React.useState(null);
  const [slideIdx, setSlideIdx] = React.useState(0);
  const slideTimerRef = React.useRef(null);

  const primary = React.useMemo(() => {
    // Priority: first variant image → first in product_image_collection → fallback
    const v0 = data?.product_variants?.[0]?.images?.[0];
    const c0 = data?.product_image_collection?.[0];
    const fallback = img; // parent-provided fallback
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
                {pageSlice.map((data) => {
                  ...
                  // ---------- replace inline card with ProductCard (no line removals, just wrapped) ----------
                  return (
                    <ProductCard
                      key={data._id}
                      data={data}
