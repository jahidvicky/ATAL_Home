...
export default function EyewearTips() {
  ...
  return (
    <section className="bg-black bg-gradient-to-b from-black to-red-600 text-white pb-16 px-4 sm:px-6 md:px-6 text-center">
      ...
      <p className="max-w-3xl mx-auto text-gray-200 mb-10 text-sm sm:text-base px-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:mx-20 mx-2 sm:mx-6">
        {eyewearTips.map((tip, index) => (
          <div
            key={index}
            className="group bg-white text-black p-5 sm:p-6 rounded-xl shadow-xl hover:shadow-lg transition-all hover:scale-105 hover:cursor-pointer hover:-translate-y-2 hover:bg-black hover:text-white border-2 border-red-600 hover:border-black"
          >
            ...
            {/* FIX HERE */}
            <p className="mb-4 text-gray-700 text-sm sm:text-base transition-colors duration-300 group-hover:text-white">
              {tip.description}
            </p>

            <Link to={`/${eyewearLinks[index]}`}>
              <button className="mt-auto border border-red-500 text-[#f00000] px-4 py-1 rounded-full transition-all duration-300 group-hover:bg-red-500 group-hover:text-white">
                Learn More →
              </button>
            </Link>
          </div>
